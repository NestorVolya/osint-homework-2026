...
class PaddleWorker(BaseWorker):
    ...
    # PaddleOCR does not have dedicated models for every language — map to the
    # closest supported script model. Ukrainian/Russian → "cyrillic" is the
    # standard recommendation from PaddleOCR docs.
    _LANG_MAP = {
        "uk": "cyrillic",
        "ru": "cyrillic",
        "bg": "cyrillic",
    ...
    @classmethod
    def _resolve_lang(cls, lang: str) -> str:
        """Resolve language code to PaddleOCR model name."""
        return cls._LANG_MAP.get(lang, lang)
    ...
    def _get_engine(cls, lang: str = "en", use_angle_cls: bool = True):
        ...
        """
        paddle_lang = cls._resolve_lang(lang)
        if cls._ocr_engine is None or cls._lang != paddle_lang:
            try:
            ...
            if lang != paddle_lang:
                logger.info(f"Language '{lang}' mapped to PaddleOCR model '{paddle_lang}'")
            logger.info(f"Initializing PaddleOCR (lang={paddle_lang}, angle_cls={use_angle_cls})")
            ...
            try:
                ...
                cls._lang = paddle_lang
                logger.info("PaddleOCR initialized")
            except Exception as e:
                if offline_mode:
                    logger.error(
                        f"Failed to initialize PaddleOCR in offline mode. "
                        f"OCR models may not be cached locally for lang={paddle_lang}. "
